...
def generate_image(
    ...
    try:
        ...
        # Báo lỗi rõ ràng nếu API key bị Google hiểu như OAuth token.
        if (
            "ACCESS_TOKEN_TYPE_UNSUPPORTED" in error_text
            or "UNAUTHENTICATED" in error_text
        ):
            raise RuntimeError(
                "Gemini trả về lỗi xác thực 401. "
                "API key hiện tại không được API chấp nhận. "
                "Hãy tạo API key mới trong Google AI Studio "
                "và cập nhật GEMINI_API_KEY trong Streamlit Secrets."
            ) from e

        raise RuntimeError(
            f"Lỗi Gemini API: {error_text}"
        ) from e
