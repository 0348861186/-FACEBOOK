...
TEXT_MODEL = "gemini-2.5-flash"  # Standard current model name
...
def get_gemini_client():
    ...
    if not api_key:
        raise RuntimeError(
            ...
            "Hãy thêm GEMINI_API_KEY vào Streamlit Secrets hoặc biến môi trường."
        )
...
def generate_background(
    ...
    # Sử dụng API chuẩn của google-genai để tạo ảnh (generate_images)
    try:
        ...
        # Dự phòng nếu môi trường dùng SDK cú pháp khác hoặc trả về tương tác
        pass

    # Fallback dự phòng nếu sử dụng Imagen / model khác qua generate_content hoặc interactions
    try:
        ...
        if image_data:
            data_val = getattr(image_data, "data", image_data)
    ...
    raise RuntimeError("Không thể tạo ảnh nền từ AI Model. Vui lòng kiểm tra model name hoặc API key.")
